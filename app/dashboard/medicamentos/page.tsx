...
import { ArrowLeft, Upload, Search, Trash2, Plus, Pill, Loader2, Save } from 'lucide-react';
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
        ...
        for (const line of lines) {
            ...
            // Se não funcionou bem com vírgula, tenta ponto e vírgula
            if (parts.length < 2) {
            ...
            // Aceita se tiver pelo menos 2 colunas (Nome e Dosagem)
            if (cleanParts.length >= 2) { 
                const nome = cleanParts[0];
                const dosagem = cleanParts[1];
                const posologia = cleanParts[2] || null; // Coluna 3 (opcional)
                const indicacao = cleanParts[3] || null; // Coluna 4 (opcional)
                ...
                if (!error) count++;
                    ...
                    console.error("Erro na linha:", cleanLine, error.message);
                    erros++;
                }
            } else {
                 erros++;
    ...
    // CORREÇÃO AQUI: Mudado para UTF-8 para consertar os acentos
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="relative flex-1">
                ...
                <input 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white shadow-sm text-gray-900"
                />
            </div>
            <div className="flex gap-2">
                ...
                <button 
                    ...
                    disabled={importing}
                    className="bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 px-4 py-2.5 rounded-lg font-medium flex items-center gap-2 shadow-sm disabled:opacity-50 transition-colors"
                >
                    {importing ? <Loader2 className="animate-spin"/> : <Upload size={20} />} Importar TXT
                </button>
                <button 
                    onClick={() => setIsModalOpen(true)}
                    className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2.5 rounded-lg font-medium flex items-center gap-2 shadow-md transition-all active:scale-95"
                >
                    <Plus size={20} /> Novo
                </button>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead className="bg-orange-50/50 text-orange-800 text-sm uppercase">
                        <tr>
                            ...
                            <th className="p-4">Indicação / Posologia</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                             ...
                             <tr><td colSpan={4} className="p-8 text-center text-gray-400">Nenhum medicamento encontrado.</td></tr>
                        ) : (
                            filteredMeds.map(med => (
                                <tr key={med.id} className="hover:bg-orange-50/30 transition-colors">
                                    ...
                                    <td className="p-4 text-gray-500 text-sm truncate max-w-xs" title={`${med.indicacao || ''} ${med.posologia || ''}`}>
                                        <div className="flex flex-col">
                                            {med.indicacao && <span>{med.indicacao}</span>}
                                            {med.posologia && <span className="text-xs text-gray-400">{med.posologia}</span>}
            ...
            <div className="bg-white rounded-xl p-6 w-full max-w-sm shadow-2xl">
                ...
                <form onSubmit={handleSalvarManual} className="space-y-4">
                    <div>
                        ...
                        <input autoFocus className="w-full border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none text-gray-900" placeholder="Ex: Dipirona" value={novoNome} onChange={e => setNovoNome(e.target.value)} />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Dosagem</label>
                        <input className="w-full border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none text-gray-900" placeholder="Ex: 500mg" value={novaDosagem} onChange={e => setNovaDosagem(e.target.value)} />
                    </div>
                    <div className="flex justify-end gap-2 pt-2">
                        ...
                        <button type="submit" className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2 shadow-sm"><Save size={16}/> Salvar</button>
                    </div>
                </form>
            </div>
        </div>
