...
import { LogOut, Plus, Search, AlertCircle, Save, Loader2, Upload, Clock, X, UserCheck, UserX, Users, Pill, Trash2, Lock, AlertTriangle } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const handleZerarSistema = async (e: React.FormEvent) => {
    ...
    // Deleta todos os encontristas (prescrições e histórico somem via Cascade do Banco)
    // .gt('id', 0) é um truque para deletar "tudo onde id > 0"
    const { error } = await supabase.from('encontristas').delete().gt('id', 0);

    if (error) {
        alert("Erro ao limpar o sistema: " + error.message);
    } else {
        alert("Sistema limpo com sucesso! Pronto para o próximo encontro.");
        setEncontristas([]); // Limpa a tela
    ...
    <div className="min-h-screen bg-orange-50 relative">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Nome, Responsável ou ID exato..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 shadow-sm text-gray-900" />
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
             ...
             <form onSubmit={handleSalvar} className="p-6 space-y-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Nome Completo *</label><input type="text" value={novoNome} onChange={e => setNovoNome(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-gray-900" placeholder="Ex: João da Silva" autoFocus /></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Responsável</label><input type="text" value={novoResponsavel} onChange={e => setNovoResponsavel(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-gray-900" placeholder="Ex: Pr. Mario" /></div>
                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Alergias</label><input type="text" value={novasAlergias} onChange={e => setNovasAlergias(e.target.value)} className="w-full px-3 py-2 border border-red-200 bg-red-50 rounded-lg text-gray-900" placeholder="Ex: Dipirona" /></div>
                </div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Observações</label><textarea rows={3} value={novasObservacoes} onChange={e => setNovasObservacoes(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-gray-900" placeholder="..." /></div>
      ...
      {isResetModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 border-2 border-red-100">
             <div className="bg-red-50 px-6 py-6 flex flex-col items-center text-center border-b border-red-100">
               ...
               <p className="text-red-700 text-sm mt-2">
                 Cuidado! Isso apagará <strong>TODOS</strong> os encontristas, históricos e prescrições. A base de medicamentos será mantida.
               </p>
             </div>
             
             <form onSubmit={handleZerarSistema} className="p-6 space-y-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Digite a senha de confirmação:</label>
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="w-full pl-9 pr-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent text-gray-900" 
                            placeholder="Senha..." 
                            autoFocus 
                        />
                    </div>
                ...
                <div className="flex gap-3 pt-2">
                    <button type="button" onClick={() => {setIsResetModalOpen(false); setResetPassword('')}} className="flex-1 px-4 py-2.5 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg font-medium transition-colors">Cancelar</button>
                    <button type="submit" disabled={isResetting} className="flex-1 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold shadow-sm flex justify-center items-center gap-2 disabled:opacity-50 transition-colors">
                        {isResetting ? <Loader2 className="animate-spin h-4 w-4"/> : 'Confirmar Limpeza'}
                    </button>
                </div>
             </form>
          </div>
