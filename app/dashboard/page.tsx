...
export default function Dashboard() {
  ...
  const getStatusPessoa = (pessoa: Encontrista) => {
    ...
    let statusGeral = 3; // 3=Atrasado (pior), 2=Atenção, 1=Em dia
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
                type="text" 
                placeholder="Nome, Responsável ou ID exato..." 
                value={searchTerm} 
                      ...
                      <td className="p-4">
                        {pessoa.alergias ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 border border-red-200">
                                <AlertCircle size={12} /> {pessoa.alergias}
                            </span>
                        ) : (
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
             ...
             <form onSubmit={handleSalvar} className="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" value={novoNome} onChange={e => setNovoNome(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-gray-900 focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Ex: João da Silva" autoFocus />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Responsável</label>
                        <input type="text" value={novoResponsavel} onChange={e => setNovoResponsavel(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-gray-900 focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Ex: Pr. Mario" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Alergias</label>
                        <input type="text" value={novasAlergias} onChange={e => setNovasAlergias(e.target.value)} className="w-full px-3 py-2 border border-red-200 bg-red-50 rounded-lg text-gray-900 focus:ring-2 focus:ring-red-500 focus:outline-none" placeholder="Ex: Dipirona" />
                    </div>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Observações</label>
                    <textarea rows={3} value={novasObservacoes} onChange={e => setNovasObservacoes(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-gray-900 focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="..." />
                </div>
                <div className="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" disabled={saving} className="px-4 py-2 bg-orange-600 text-white rounded-lg font-medium shadow-sm flex items-center gap-2 disabled:opacity-50 hover:bg-orange-700 transition-colors">
