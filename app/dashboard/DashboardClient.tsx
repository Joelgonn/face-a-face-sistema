...
// --------------------------------------------------------------
export type ShrinkedEncontrista = {
  id: number;
  nome: string;
  check_in: boolean;
...
async function safeQuery<T>(fn: () => Promise<T>): Promise<T | null> {
  try {
    ...
    if (result && typeof result === 'object') {
      ...
      if (possibleError.error) throw possibleError.error;
    }
    return result;
  } catch (err) {
...
export default function DashboardClient({ initialEncontristas, isAdminInitial }: DashboardClientProps) {
  ...
  const [encontristas, setEncontristas] = useState<EncontristaDashboard[]>(() => {
    ...
    if (cached) {
      return cached.map(expandEncontristaFromCache);
    }
    return [];
  });
  ...
  useEffect(() => {
    ...
    if (navigator.serviceWorker?.controller) {
      navigator.serviceWorker.controller.postMessage({ type: 'CACHE_DASHBOARD', url: '/dashboard' });
    }
  }, []);
  ...
  async function preloadInBatches(ids: number[], signal?: AbortSignal) {
  ...
  useEffect(() => {
    ...
    preloadInBatches(encontristas.map(e => e.id), abortController.signal)
      .then(() => {
        if (!abortController.signal.aborted) markCacheReady(runId);
      })
      .catch(err => {
  ...
  useEffect(() => {
    ...
    if (initialEncontristas.length === 0 && !hasFastCache) {
  ...
  // ============================================================
  useEffect(() => {
    const handleResize = () => setModoSimples(window.innerWidth < 768);
  ...
  useEffect(() => {
    setIsTyping(true);
    const timer = setTimeout(() => { setDebouncedSearch(searchTerm); setIsTyping(false); }, 300);
  ...
  const filtered = useMemo(() => {
    ...
    if (!term) return normalizedEncontristas.map(toOriginalEncontrista);
    const isNumeric = /^\d+$/.test(term);
    if (isNumeric) {
      const exact = normalizedEncontristas.filter(p => p.id_str === term);
      return exact.map(toOriginalEncontrista);
    }
    return normalizedEncontristas.filter(p => p.nome_lower.includes(term) || p.responsavel_lower.includes(term)).map(toOriginalEncontrista);
  }, [normalizedEncontristas, debouncedSearch, toOriginalEncontrista]);

  const updateQueueCount = useCallback(() => setQueueCount(queueService.getQueueCount()), []);
  ...
  // ============================================================
  // ONLINE/OFFLINE E SYNC AUTOMÁTICO
  // ============================================================
  useEffect(() => {
    let interval: NodeJS.Timeout;
    const startSyncLoop = () => {
      interval = setInterval(() => {
        if (navigator.onLine && queueService.getQueueCount() > 0) syncOfflineData();
      }, 10000);
    };
    startSyncLoop();
  ...
  useEffect(() => {
    const update = () => {
      ...
      else showToast('warning', 'Sem internet', 'Modo offline ativado.');
    };
    update();
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent<{ type?: string }>) => {
      if (event.data?.type === 'SYNC_OFFLINE_DATA' && navigator.onLine) void syncOfflineData();
    };
  ...
  const toggleModoSimples = useCallback(() => {
    startTransition(() => setModoSimples(prev => !prev));
  }, []);
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'encontristas' }, (payload) => {
        if (isNavigating) return;
  ...
  useEffect(() => {
    ...
    if (/^\d+$/.test(term)) {
      ...
      if (exact) {
        setFlashId(exact.id);
        const timeout = setTimeout(() => setFlashId(null), 800);
  ...
  const confirmCheckIn = async () => {
    ...
    if (modoEmergencia) { showToast('warning', 'Modo emergência', 'Ação bloqueada'); return; }
    ...
    const result = await toggleCheckin(
      ...
      {
        updateRemote: async (id, status) => await supabase.from('encontristas').update({ check_in: status }).eq('id', id),
        addToQueue: (item) => { queueService.enqueue(item); updateQueueCount(); }
      }
    );
    if (result.queued) showToast('warning', 'Offline', 'Check-in salvo localmente.');
    else if (result.shouldRollback) { setEncontristas(backup); showToast('error', 'Erro', 'Falha ao atualizar no servidor'); }
  };
  const cancelCheckIn = () => setCheckInTarget(null);
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    const result = await criarEncontrista(
      ...
      {
        insertRemote: async (data) => await supabase.from('encontristas').insert(data),
        addToQueue: (item) => { queueService.enqueue(item); updateQueueCount(); }
      }
    );
    if (result.queued) {
  ...
  const processFileImport = () => {
    ...
    Papa.parse(fileToImport, {
      ...
      complete: async (results) => {
        const registros: { nome: string; alergias: string | null; observacoes: string | null; responsavel: string | null; check_in: boolean; }[] = [];
        for (const parts of results.data as string[][]) {
          if (parts.length >= 2 && !(parts[0] && parts[0].trim().startsWith('#'))) {
            const nome = parts[1]?.trim();
            if (nome) registros.push({ nome, alergias: parts[2]?.trim() || null, observacoes: parts[3]?.trim() || null, responsavel: parts[4]?.trim() || null, check_in: false });
          }
        }
      ...
      error: () => { showToast('error', 'Erro de Leitura', 'Falha na leitura do arquivo.'); setImporting(false); }
    });
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white pb-24">
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-100 px-4 py-4 md:px-8">
        <div className="max-w-6xl mx-auto flex justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            {showInstallButton && (
              <motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={handleInstallClick} className="px-3 py-2 rounded-xl bg-blue-500 text-white text-xs font-bold hover:bg-blue-600 transition-colors flex items-center gap-1 shadow-md">
                📱 Instalar App
              </motion.button>
            )}
            <button onClick={() => setChatbotOpen(true)} className="hidden md:flex px-3 py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 items-center gap-2">
              <MessageCircle size={16} /> Chat
            </button>
            <div className="hidden md:flex items-center gap-1 rounded-2xl border border-slate-200 bg-white p-1 shadow-sm">
              <motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={() => { if (!modoSimples) toggleModoSimples(); }} disabled={isPending} className={`flex h-10 items-center gap-2 rounded-xl px-3 text-xs font-black transition-all disabled:opacity-50 ${modoSimples ? 'bg-orange-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-100'}`}>
                <Smartphone size={16} /><span className="hidden sm:inline">Simples</span>
              </motion.button>
              <motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={() => { if (modoSimples) toggleModoSimples(); }} disabled={isPending} className={`flex h-10 items-center gap-2 rounded-xl px-3 text-xs font-black transition-all disabled:opacity-50 ${!modoSimples ? 'bg-slate-700 text-white shadow-md' : 'text-slate-500 hover:bg-slate-100'}`}>
                <Monitor size={16} /><span className="hidden sm:inline">Completo</span>
              </motion.button>
            </div>
            <div className={`hidden sm:block px-3 py-1.5 rounded-full text-xs font-bold ${connectionStatus.bg}`}>
              {connectionStatus.icon} {connectionStatus.text} {queueCount > 0 && <span className="ml-1 font-black">({queueCount})</span>}
            </div>
            <div className="relative">
              <motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={() => setOpenMenu(!openMenu)} className="p-2.5 bg-gradient-to-br from-orange-500 to-orange-600 text-white rounded-xl shadow-md">
                ☰
              </motion.button>
              <AnimatePresence>
                {openMenu && (
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-8 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={fastTransition} className="bg-white rounded-2xl shadow-sm border border-slate-300 overflow-hidden">
            <button onClick={() => setShowStats(!showStats)} className="w-full px-5 py-3.5 flex items-center justify-between hover:bg-slate-50 transition-colors">
              ...
              <div className="flex items-center gap-4">
                {!showStats && <div className="hidden sm:flex items-center gap-6 text-[10px] font-black uppercase tracking-widest"><span className="text-slate-400">Inscritos: <span className="text-slate-700 text-xs ml-1">{totalEncontristas}</span></span><span className="text-emerald-500">Presentes: <span className="text-emerald-600 text-xs ml-1">{totalPresentes}</span></span><span className="text-rose-500">Ausentes: <span className="text-rose-600 text-xs ml-1">{totalAusentes}</span></span></div>}
              ...
              {showStats && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={fastTransition} className="border-t border-slate-100">
                  <div className="grid grid-cols-3 divide-x divide-slate-100 bg-slate-50/50">
                    <div className="p-4 flex flex-col items-center justify-center text-center"><Users size={20} className="text-blue-500 mb-1" /><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Inscritos</p><p className="text-2xl font-black text-slate-800">{totalEncontristas}</p></div>
                    <div className="p-4 flex flex-col items-center justify-center text-center"><UserCheck size={20} className="text-emerald-500 mb-1" /><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Presentes</p><p className="text-2xl font-black text-emerald-600">{totalPresentes}</p></div>
                    <div className="p-4 flex flex-col items-center justify-center text-center"><UserX size={20} className="text-rose-500 mb-1" /><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ausentes</p><p className="text-2xl font-black text-rose-600">{totalAusentes}</p></div>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={fastTransition} className="px-0 mt-0">
            <div className="flex items-center justify-between bg-white border border-slate-300 shadow-sm rounded-2xl px-4 py-3">
              <div className="flex items-center gap-4 text-[11px] font-black tracking-wide">
                ...
                <motion.span key={totalAusentes} initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 500, damping: 30 }} className="text-rose-600">❌ {totalAusentes}</motion.span>
              </div>
              <div className={`text-[10px] px-2 py-1 rounded-full font-bold ${connectionStatus.bg}`}>{connectionStatus.icon} {connectionStatus.text}{queueCount > 0 && <span className="ml-1 bg-white/50 px-1 rounded-full">{queueCount}</span>}</div>
            </div>
          </motion.div>
        )}
        ...
        <div className="flex flex-col gap-3">
          <div className="relative flex-1">
            ...
            <input ref={inputRef} type="text" placeholder={modoSimples ? "Buscar por nome ou ID..." : "Buscar por nome, responsável ou ID..."} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-5 bg-white border border-slate-200 rounded-2xl focus:ring-4 focus:ring-orange-500/10 font-medium text-slate-700 shadow-sm transition-all text-base" />
            {isTyping && <div className="absolute right-4 top-1/2 -translate-y-1/2"><Loader2 size={18} className="text-orange-500 animate-spin" /></div>}
            {!isTyping && searchTerm && <div className="absolute right-4 top-1/2 -translate-y-1/2"><span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded-full">{filtered.length}</span></div>}
          </div>
        </div>

        {searchTerm && !isTyping && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-2">
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider">{filtered.length === 0 ? 'Nenhum resultado encontrado' : `${filtered.length} ${filtered.length === 1 ? 'resultado' : 'resultados'}`}</p>
          </motion.div>
        )}
        ...
        {modoSimples && (
          <div className="space-y-3">
            <AnimatePresence mode="wait">
              {!selectedPatient && (
                <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={fastTransition}>
                  {loading ? (
                    ...
                    <motion.div animate={{ scale: selectedPatient ? 0.96 : 1, filter: selectedPatient ? 'blur(3px)' : 'blur(0px)' }} transition={fastTransition}>
                      {sorted.map((pessoa, index) => (
                        <motion.div key={pessoa.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.025, duration: 0.18, ease: premiumEasing }}>
                          <PatientCard id={pessoa.id} nome={pessoa.nome} responsavel={pessoa.responsavel} alergias={pessoa.alergias} status={statusMap.get(pessoa.id)} checkIn={pessoa.check_in || false} flashId={flashId} onCheckIn={handlePatientCheckIn} onClick={() => setSelectedPatient(pessoa)} />
                        </motion.div>
                      ))}
                    </motion.div>
                  )}
              ...
              {selectedPatient && <PatientDetail key="detail" paciente={selectedPatient} onClose={() => setSelectedPatient(null)} />}
            </AnimatePresence>
          </div>
        )}

        {!modoSimples && (
          <motion.div animate={{ scale: selectedPatient ? 0.96 : 1, filter: selectedPatient ? 'blur(3px)' : 'blur(0px)' }} transition={fastTransition} className="space-y-3">
            ...
            {loading ? (
              ...
              <AnimatePresence><div className="space-y-2">{sorted.map((pessoa, index) => {
                ...
                let pillBgClass = '', pillTextClass = 'text-orange-400', pillBorderClass = '';
                if (isEmDia) { pillBgClass = 'bg-emerald-200'; pillBorderClass = 'border-emerald-300'; }
                else if (isAtrasado) { pillBgClass = 'bg-rose-200'; pillBorderClass = 'border-rose-300'; }
                else if (isSemMeds) { pillBgClass = 'bg-gray-200'; pillBorderClass = 'border-gray-300'; }
                else { pillBgClass = status?.cor || 'bg-slate-100'; pillTextClass = status?.cor?.includes('orange') ? 'text-orange-600' : 'text-slate-600'; pillBorderClass = 'border-transparent'; }
                const customPillColor = `${pillBgClass} ${pillTextClass} ${pillBorderClass} border`;
                return (
                  <motion.div key={pessoa.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.02, duration: 0.18, ease: premiumEasing }} className={`group grid grid-cols-12 gap-4 items-center bg-white hover:bg-orange-50/40 rounded-xl border-l-4 ${status?.bordaL || 'border-l-slate-300'} border-y border-r border-slate-100 shadow-sm hover:shadow-md transition-all duration-200 p-3 px-5 cursor-pointer`} onClick={() => handleNavigateToPatientPage(pessoa.id)}>
                    <div className="col-span-2"><span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider border ${customPillColor}`}>{status?.icone} {status?.texto}</span></div>
                    <div className="col-span-4 flex items-center gap-3 min-w-0"><div className="w-8 h-8 bg-orange-100 border border-orange-200 rounded-full flex items-center justify-center font-black text-orange-700 text-xs shrink-0 shadow-inner group-hover:bg-orange-500 group-hover:text-white transition-colors duration-200">{pessoa.id}</div><span className="font-bold text-slate-800 text-base group-hover:text-orange-600 transition-colors truncate">{pessoa.nome}</span></div>
                    <div className="col-span-2 flex justify-center"><motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={(e) => { e.stopPropagation(); handlePatientCheckIn(pessoa.id, pessoa.check_in || false, pessoa.nome); }} className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-black border shadow-sm transition-all ${pessoa.check_in ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>{pessoa.check_in ? <UserCheck size={14} /> : <UserX size={14} />} {pessoa.check_in ? 'Presente' : 'Ausente'}</motion.button></div>
                    <div className="col-span-2 text-slate-500 font-medium text-sm truncate pr-3">{pessoa.responsavel || <span className="text-slate-300">-</span>}</div>
                    <div className="col-span-2 min-w-0">{pessoa.alergias ? <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-[9px] font-bold bg-rose-50 text-rose-600 border border-rose-100 max-w-full" title={pessoa.alergias}><AlertCircle size={12} className="shrink-0" /> <span className="truncate">{pessoa.alergias}</span></span> : <span className="text-slate-300 text-[11px] font-medium">-</span>}</div>
                  </motion.div>
                );
              })}</div></AnimatePresence>
            )}
      ...
      {modoSimples && !chatbotOpen && (
        <div className="fixed bottom-6 right-6 z-40">
          <AnimatePresence>
            {fabOpen && (
              <motion.div initial={{ opacity: 0, y: 20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.9 }} transition={fastTransition} className="flex flex-col items-end gap-2 mb-4">
                <motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={() => { setChatbotOpen(true); setFabOpen(false); }} className="bg-white text-slate-700 shadow-lg px-5 py-3 rounded-2xl font-bold flex items-center gap-2 text-sm border border-slate-200"><MessageCircle size={18} className="text-orange-500" /> Chat</motion.button>
                <motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={() => { setIsModalOpen(true); setFabOpen(false); }} className="bg-white text-slate-700 shadow-lg px-5 py-3 rounded-2xl font-bold flex items-center gap-2 text-sm border border-slate-200"><Plus size={18} className="text-emerald-500" /> Novo Paciente</motion.button>
                {isAdmin && (<motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={() => { fileInputRef.current?.click(); setFabOpen(false); }} className="bg-white text-slate-700 shadow-lg px-5 py-3 rounded-2xl font-bold flex items-center gap-2 text-sm border border-slate-200"><Upload size={18} className="text-blue-500" /> Importar Lista</motion.button>)}
                {isAdmin && (<motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={() => { setIsResetModalOpen(true); setFabOpen(false); }} className="bg-white text-rose-600 shadow-lg px-5 py-3 rounded-2xl font-bold flex items-center gap-2 text-sm border border-rose-200"><Trash2 size={18} /> Zerar Sistema</motion.button>)}
              </motion.div>
            )}
          </AnimatePresence>
          <motion.button onClick={() => setFabOpen(prev => !prev)} whileTap={{ scale: 0.9 }} transition={fastTransition} animate={{ rotate: fabOpen ? 45 : 0 }} className="w-14 h-14 bg-gradient-to-br from-orange-500 to-orange-600 text-white rounded-full shadow-lg shadow-orange-500/30 flex items-center justify-center"><Plus size={24} /></motion.button>
        </div>
      )}

      <ChatbotWidget isOpen={chatbotOpen} onOpenChange={(isOpen) => { setChatbotOpen(isOpen); if (isOpen) setFabOpen(false); }} />
        ...
        {showQueue && !modoSimples && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={fastTransition} className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={springTransition} className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-100">
              <div className="p-5 border-b border-slate-100 flex justify-between items-center"><h2 className="font-black text-slate-800 flex items-center gap-2">☁️ Pendências Offline {queueCount > 0 && <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{queueCount}</span>}</h2><motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={() => setShowQueue(false)} className="p-2 hover:bg-slate-100 rounded-full"><X size={18} /></motion.button></div>
              <div className="p-5"><div className="max-h-80 overflow-y-auto space-y-2 mb-5">{queueService.getQueue().length === 0 ? (<div className="text-center py-8 text-slate-400"><CheckCircle2 className="w-12 h-12 mx-auto mb-2 text-emerald-400" /><p className="font-medium">Nenhuma pendência</p><p className="text-xs">Tudo sincronizado</p></div>) : (queueService.getQueue().map((item, i) => { const summary = getQueueItemSummary(item); return (<motion.div key={item.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05, duration: 0.12, ease: premiumEasing }} className="text-sm border border-slate-200 p-3 rounded-xl bg-slate-50"><div className="flex items-center gap-2"><div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${summary.accent}`}>{summary.icon}</div><div><p className="font-bold text-slate-700">{summary.title}</p><p className="text-xs text-slate-500">{summary.detail}</p></div></div><div className="text-[9px] text-slate-400 mt-1 flex items-center justify-between"><span>{new Date(item.createdAt).toLocaleTimeString()}</span><span>{item.status === 'failed' ? `falhou ${item.retryCount}x` : `${item.status} • ${item.retryCount}x`}</span></div></motion.div>);}))}</div><div className="flex gap-3"><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} onClick={async () => { await syncOfflineData(); setShowQueue(false); }} disabled={queueCount === 0} className={`flex-1 py-3 rounded-xl font-bold transition-all ${queueCount === 0 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-blue-600 text-white shadow-md hover:bg-blue-700'}`}>☁️ Sincronizar</motion.button><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} onClick={() => { if (confirm('Limpar TODAS as pendências? Essa ação não pode ser desfeita.')) { queueService.clearQueue(); updateQueueCount(); setShowQueue(false); showToast('warning', 'Fila limpa', 'Pendências removidas localmente'); } }} disabled={queueCount === 0} className={`flex-1 py-3 rounded-xl font-bold transition-all ${queueCount === 0 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-red-500 text-white shadow-md hover:bg-red-600'}`}>🗑️ Limpar</motion.button></div></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {isModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={fastTransition} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={springTransition} className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100">
              <div className="p-5 flex justify-between items-center border-b border-slate-100"><h2 className="text-xl font-black text-slate-800 flex items-center gap-2"><Plus className="text-orange-500"/> Novo Paciente</h2><motion.button whileTap={{ scale: 0.95 }} transition={fastTransition} onClick={() => setIsModalOpen(false)} className="p-2 bg-slate-100 rounded-full text-slate-500 hover:bg-slate-200"><X size={18}/></motion.button></div>
              <form onSubmit={handleSalvar} className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label className="text-[10px] font-black text-slate-500 uppercase tracking-wider pl-2 mb-1 block">Responsável</label><input type="text" value={novoResponsavel} onChange={e => setNovoResponsavel(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-medium focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all" placeholder="Opcional" /></div><div><label className="text-[10px] font-black text-rose-500 uppercase tracking-wider pl-2 mb-1 block">Alergias</label><input type="text" value={novasAlergias} onChange={e => setNovasAlergias(e.target.value)} className="w-full px-4 py-3 bg-rose-50 border border-rose-200 text-rose-700 rounded-xl font-medium focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all" placeholder="Se houver" /></div></div>
                ...
                <div className="flex justify-end gap-3 pt-3"><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 font-bold text-slate-500">Cancelar</motion.button><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} type="submit" disabled={saving} className="px-6 py-2.5 bg-gradient-to-br from-orange-500 to-orange-600 text-white rounded-xl font-bold shadow-md flex items-center gap-2 disabled:opacity-50">{saving ? <Loader2 className="animate-spin h-4 w-4"/> : <><Save size={16}/> Cadastrar</>}</motion.button></div>
              </form>
            </motion.div>
          </motion.div>
        )}
        ...
        {isImportConfirmOpen && fileToImport && !modoSimples && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={fastTransition} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={springTransition} className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center border border-slate-100">
              ...
              <div className="flex gap-3"><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} onClick={() => setIsImportConfirmOpen(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl font-bold">Cancelar</motion.button><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} onClick={processFileImport} disabled={importing} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold shadow-md">{importing ? <Loader2 className="animate-spin h-5 w-5 mx-auto" /> : 'Importar'}</motion.button></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {checkInTarget && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={fastTransition} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={springTransition} className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center border border-slate-100">
              <div className={`w-14 h-14 rounded-xl flex items-center justify-center mx-auto mb-4 ${checkInTarget.status ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'}`}>{checkInTarget.status ? <UserX size={28} /> : <UserCheck size={28} />}</div>
              ...
              <div className="flex gap-3"><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} onClick={cancelCheckIn} className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl font-bold">Cancelar</motion.button><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} onClick={confirmCheckIn} className={`flex-1 py-3 rounded-xl font-bold text-white shadow-md ${checkInTarget.status ? 'bg-rose-500' : 'bg-emerald-600'}`}>Confirmar</motion.button></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {isResetModalOpen && !modoSimples && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={fastTransition} className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={springTransition} className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 text-center border border-rose-100">
              ...
              <form onSubmit={handleZerarSistema} className="space-y-4"><input type="password" value={resetPassword} onChange={e => setResetPassword(e.target.value)} className="w-full px-4 py-3 bg-rose-50/50 border border-rose-200 rounded-xl text-center font-bold text-rose-700 focus:ring-2 focus:ring-rose-500/20 placeholder:text-rose-300" placeholder="Digite sua senha de admin" />{resetError && <p className="text-rose-600 font-bold text-xs bg-rose-50 py-2 rounded-xl">{resetError}</p>}<div className="flex gap-3 pt-2"><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} type="button" onClick={() => setIsResetModalOpen(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl font-bold">Cancelar</motion.button><motion.button whileTap={{ scale: 0.97 }} transition={fastTransition} type="submit" disabled={isResetting} className="flex-1 py-3 bg-rose-600 text-white rounded-xl font-bold shadow-md">{isResetting ? <Loader2 className="animate-spin h-5 w-5 mx-auto" /> : 'Apagar Tudo'}</motion.button></div></form>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
